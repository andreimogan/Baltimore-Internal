...
const STATUS_STYLES = {
  ...
  Cancelled: { bg: 'var(--ui-border)', text: 'var(--color-white)' },
}
...
function Badge({ label, styleMap }) {
  const style = styleMap[label] || { bg: 'var(--ui-border)', text: 'var(--color-white)' }
  return (
  ...
  return (
    ...
    >
      <div className="w-full px-8 py-6">
        <div className="mb-5">
          <p className="text-2xl font-bold mb-1" style={{ color: 'var(--ui-text-primary)' }}>
            Work Orders
          </p>
          <p className="text-xs uppercase tracking-wide" style={{ color: 'var(--ui-text-muted)' }}>
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          {[
            { label: 'Total Orders', value: stats.total, color: 'var(--ui-accent)' },
            ...
            <div key={metric.label} className="rounded-[10px] border px-3 py-2.5" style={{ borderColor: 'var(--ui-border)', background: 'var(--ui-surface-muted)' }}>
              <p className="text-[10px] uppercase tracking-wide mb-1" style={{ color: 'var(--ui-text-muted)' }}>{metric.label}</p>
              <p className="text-2xl font-bold leading-none" style={{ color: metric.color }}>{metric.value}</p>
            </div>
          ))}
        </div>

        <div className="rounded-[10px] border overflow-hidden" style={{ borderColor: 'var(--ui-border)', background: 'var(--ui-surface-muted)' }}>
          <div
          ...
          {hydratedRows.length === 0 ? (
            <div className="text-center py-10" style={{ color: 'var(--ui-text-muted)' }}>
            ...
            hydratedRows.map((row, index) => {
              ...
              return (
                <div key={row.id} className="border-b" style={{ borderColor: index === hydratedRows.length - 1 ? 'transparent' : 'var(--ui-border)' }}>
                  <div
                  ...
                  >
                    <button
                      ...
                      style={{ color: 'var(--ui-text-muted)', background: 'var(--ui-surface-muted)' }}
                      onClick={() => setExpandedId(expanded ? null : row.id)}
                    >
                      {expanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                    </button>
                    <span className="font-mono font-semibold truncate pr-2 min-w-0" style={{ color: 'var(--ui-accent)' }}>{row.id}</span>
                    <span className="truncate pr-2 min-w-0" style={{ color: 'var(--ui-text-secondary)' }}>{row.type}</span>
                    ...
                    <span className="truncate min-w-0" style={{ color: 'var(--ui-text-muted)' }}>{formatDateTime(row.createdAt)}</span>
                    <span className="truncate pl-2 min-w-0" style={{ color: 'var(--ui-text-muted)' }}>{row.location || '-'}</span>
                    <span className="pl-2 min-w-0">
                      <button
                        type="button"
                        className="text-[11px] underline underline-offset-2"
                        style={{
                          color: row.mapLocation?.lng && row.mapLocation?.lat ? 'var(--ui-accent)' : 'var(--ui-text-muted)',
                  ...
                  {expanded && (
                    <div className="px-4 pb-3 pt-1 text-xs border-t" style={{ borderColor: 'var(--ui-border)', background: 'var(--ui-surface-muted)' }}>
                      <div className="flex items-center gap-1.5 mb-1.5" style={{ color: 'var(--ui-text-secondary)' }}>
                      ...
                      <div className="mt-3">
                        <p className="text-[10px] uppercase tracking-wide mb-2" style={{ color: 'var(--ui-text-muted)' }}>
                          Progress Steps
                        </p>
                              ...
                              <>
                                {actionItems.map((actionText, idx) => (
                                  <button
                                    ...
                                    style={{
                                      borderColor: 'var(--ui-border)',
                                      background: currentState[idx] ? 'var(--ui-status-success-bg)' : 'var(--ui-surface-muted)',
                                      color: currentState[idx] ? 'var(--ui-text-secondary)' : 'var(--ui-text-secondary)',
                                    }}
                                  >
                                    {currentState[idx] ? (
                                      <CheckCircle2 className="w-3.5 h-3.5 mt-0.5 shrink-0 text-green-400" />
                                    ) : (
                                      <Square className="w-3.5 h-3.5 mt-0.5 shrink-0" style={{ color: 'var(--ui-text-muted)' }} />
                                    )}
                                ...
                                <div className="mt-2 px-1">
                                  <div className="flex items-center justify-between mb-1">
                                    <span className="text-[10px] uppercase tracking-wide" style={{ color: 'var(--ui-text-muted)' }}>
                                      Completion
                                    </span>
                                    <span className="text-[10px] font-semibold" style={{ color: 'var(--ui-accent)' }}>
                                      {completedCount}/{actionItems.length} ({percent}%)
                                    </span>
                                  </div>
                                  <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--ui-border-subtle)' }}>
                                    <div
                      ...
                      <div className="mt-2 flex items-center gap-1.5 text-[10px]" style={{ color: 'var(--ui-text-muted)' }}>
