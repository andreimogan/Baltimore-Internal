// Calculate statistics for top affected neighborhoods

/**
 * Calculate centroid (center point) of a polygon or multipolygon
 */
function calculateCentroid(geometry) {
  if (!geometry) return null

  let coordinates = []
  
  if (geometry.type === 'Polygon') {
    coordinates = geometry.coordinates[0] // Use outer ring
  } else if (geometry.type === 'MultiPolygon') {
    // Use the largest polygon's outer ring
    const largestPolygon = geometry.coordinates.reduce((largest, current) => {
      return current[0].length > largest[0].length ? current : largest
    })
    coordinates = largestPolygon[0]
  } else {
    return null
  }

  // Calculate average of all points
  let sumLng = 0
  let sumLat = 0
  coordinates.forEach(([lng, lat]) => {
    sumLng += lng
    sumLat += lat
  })

  return [sumLng / coordinates.length, sumLat / coordinates.length]
}

/**
 * Check if a point is inside a polygon (same as neighborhoodDensity.js)
 */
function isPointInPolygon(point, geometry) {
  if (!geometry) return false

  if (geometry.type === 'Polygon') {
    return pointInPolygonRings(point, geometry.coordinates)
  } else if (geometry.type === 'MultiPolygon') {
    return geometry.coordinates.some(rings => pointInPolygonRings(point, rings))
  }
  
  return false
}

function pointInPolygonRings(point, rings) {
  const [lng, lat] = point
  const outerRing = rings[0]
  
  if (!isPointInRing(lng, lat, outerRing)) {
    return false
  }
  
  for (let i = 1; i < rings.length; i++) {
    if (isPointInRing(lng, lat, rings[i])) {
      return false
    }
  }
  
  return true
}

function isPointInRing(lng, lat, ring) {
  let inside = false
  
  for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
    const [xi, yi] = ring[i]
    const [xj, yj] = ring[j]
    
    const intersect = ((yi > lat) !== (yj > lat)) &&
      (lng < (xj - xi) * (lat - yi) / (yj - yi) + xi)
    
    if (intersect) inside = !inside
  }
  
  return inside
}

/**
 * Get color tier for a neighborhood based on density ratio
 */
function getColorTier(count, maxCount) {
  if (count === 0 || maxCount === 0) return 'none'
  
  const ratio = count / maxCount
  
  if (ratio >= 0.7) return 'red'
  if (ratio >= 0.5) return 'orange'
  if (ratio >= 0.3) return 'amber'
  if (ratio >= 0.15) return 'yellow'
  return 'gray'
}

/**
 * Which polygon contains this point (first match). Aligns with density / map labeling.
 */
export function findNeighborhoodContainingPoint(lng, lat, neighborhoodsGeoJSON) {
  if (!neighborhoodsGeoJSON?.features?.length || typeof lng !== 'number' || typeof lat !== 'number') {
    return null
  }
  const point = [lng, lat]
  for (const f of neighborhoodsGeoJSON.features) {
    if (isPointInPolygon(point, f.geometry)) {
      return f.properties?.Name ?? null
    }
  }
  return null
}

/** True when neighborhood is red-tier (critical) for the current density map. */
export function isCriticalNeighborhood(neighborhoodName, densityMap) {
  if (!neighborhoodName || !densityMap || !Object.keys(densityMap).length) return false
  const maxCount = Math.max(...Object.values(densityMap), 1)
  const count = densityMap[neighborhoodName] ?? 0
  return getColorTier(count, maxCount) === 'red'
}

/**
 * Outer ring used for bbox (same primary polygon as {@link calculateCentroid}).
 */
function getPrimaryOuterRing(geometry) {
  if (!geometry) return null
  if (geometry.type === 'Polygon') return geometry.coordinates[0]
  if (geometry.type === 'MultiPolygon') {
    const largestPolygon = geometry.coordinates.reduce((largest, current) =>
      current[0].length > largest[0].length ? current : largest
    )
    return largestPolygon[0]
  }
  return null
}

/**
 * Tooltip anchor: horizontal center of bbox, vertical midpoint of the **upper half**
 * (between bbox mid-latitude and north edge) — “middle of the above half.”
 */
function getTooltipAnchorLngLat(centroid, geometry) {
  if (!centroid) return null

  const ring = getPrimaryOuterRing(geometry)
  if (!ring?.length) return centroid

  let minLng = Infinity
  let maxLng = -Infinity
  let minLat = Infinity
  let maxLat = -Infinity

  ring.forEach(([lng, lat]) => {
    minLng = Math.min(minLng, lng)
    maxLng = Math.max(maxLng, lng)
    minLat = Math.min(minLat, lat)
    maxLat = Math.max(maxLat, lat)
  })

  if (
    !Number.isFinite(minLng) ||
    !Number.isFinite(maxLng) ||
    !Number.isFinite(minLat) ||
    !Number.isFinite(maxLat)
  ) {
    return centroid
  }

  const boxCx = (minLng + maxLng) / 2
  const midLat = (minLat + maxLat) / 2
  const upperHalfMidLat = (midLat + maxLat) / 2

  return [boxCx, upperHalfMidLat]
}

function buildNeighborhoodTooltipModel(name, count, densityMap, neighborhoodsGeoJSON, baltimore311Data, hideClosed) {
  if (!name || !count || !neighborhoodsGeoJSON || !baltimore311Data) return null

  const maxCount = Math.max(...Object.values(densityMap), 1)

  const neighborhoodFeature = neighborhoodsGeoJSON.features.find(
    (f) => f.properties.Name === name
  )

  if (!neighborhoodFeature) return null

  const centroid = calculateCentroid(neighborhoodFeature.geometry)
  if (!centroid) return null

  const anchorLngLat = getTooltipAnchorLngLat(centroid, neighborhoodFeature.geometry)

  const requestsInNeighborhood = baltimore311Data.features.filter((request) => {
    if (!request.geometry || request.geometry.type !== 'Point') return false
    if (hideClosed && request.properties.CloseDate) return false

    const [lng, lat] = request.geometry.coordinates
    return isPointInPolygon([lng, lat], neighborhoodFeature.geometry)
  })

  const typeCounts = {}
  requestsInNeighborhood.forEach((req) => {
    const type = req.properties.SRType
    if (type) {
      typeCounts[type] = (typeCounts[type] || 0) + 1
    }
  })

  const topTypes = Object.entries(typeCounts)
    .sort(([, a], [, b]) => b - a)
    .slice(0, 3)
    .map(([type, typeCount]) => ({ type, count: typeCount }))

  const avgResidentsPerNeighborhood = 2150
  const estimatedResidents = Math.round(avgResidentsPerNeighborhood * (count / maxCount) * 1.5)

  return {
    name,
    count,
    residents: estimatedResidents,
    topTypes,
    centroid: anchorLngLat,
    color: getColorTier(count, maxCount),
  }
}

/**
 * All neighborhoods with at least one open request (per densityMap), with tooltip model fields.
 */
export function getImpactedNeighborhoodTooltipModels(
  densityMap,
  neighborhoodsGeoJSON,
  baltimore311Data,
  options = {}
) {
  const { hideClosed = false } = options

  if (!densityMap || !neighborhoodsGeoJSON || !baltimore311Data) {
    return []
  }

  return Object.entries(densityMap)
    .filter(([, c]) => c > 0)
    .map(([name, count]) =>
      buildNeighborhoodTooltipModel(name, count, densityMap, neighborhoodsGeoJSON, baltimore311Data, hideClosed)
    )
    .filter(Boolean)
}

function buildStormNeighborhoodTooltipModel(name, count, densityMap, neighborhoodsGeoJSON) {
  if (!name || count == null || !neighborhoodsGeoJSON?.features?.length) return null

  const maxCount = Math.max(...Object.values(densityMap), 1)

  const neighborhoodFeature = neighborhoodsGeoJSON.features.find((f) => f.properties?.Name === name)
  if (!neighborhoodFeature) return null

  const centroid = calculateCentroid(neighborhoodFeature.geometry)
  if (!centroid) return null

  const anchorLngLat = getTooltipAnchorLngLat(centroid, neighborhoodFeature.geometry)

  const residentsAtRisk = Math.round(2150 * (count / maxCount) * 1.15)
  const confidencePercent = Math.min(95, Math.max(58, Math.round(58 + 37 * (count / maxCount))))

  return {
    name,
    count,
    residents: residentsAtRisk,
    residentsAtRisk,
    topTypes: [{ type: 'Severe weather advisory', count: 1 }],
    centroid: anchorLngLat,
    color: getColorTier(count, maxCount),
    /** Storm tooltips: forecast-style content */
    confidencePercent,
    forecastWindow: 'next 24h',
    forecastDriver: 'hcd-signs',
    predictedRequests: count,
  }
}

/** Tooltip models for storm-advisory choropleth (no 311 points). */
export function getStormAdvisoryTooltipModels(densityMap, neighborhoodsGeoJSON) {
  if (!densityMap || !neighborhoodsGeoJSON?.features?.length) return []

  return Object.entries(densityMap)
    .filter(([, c]) => c > 0)
    .map(([name, count]) =>
      buildStormNeighborhoodTooltipModel(name, count, densityMap, neighborhoodsGeoJSON)
    )
    .filter(Boolean)
}

/**
 * Get top N neighborhoods by 311 density with detailed statistics
 * @param {Object} densityMap - Map of neighborhood name to open request count
 * @param {Object} neighborhoodsGeoJSON - GeoJSON with neighborhood polygons
 * @param {Object} baltimore311Data - Full 311 GeoJSON (already filtered by enabled types)
 * @param {Object} options - { hideClosed, topN }
 * @returns {Array} Array of neighborhood stats objects
 */
export function getTopNeighborhoods(densityMap, neighborhoodsGeoJSON, baltimore311Data, options = {}) {
  const { hideClosed = false, topN = 4 } = options
  
  if (!densityMap || !neighborhoodsGeoJSON || !baltimore311Data) {
    return []
  }

  const sorted = Object.entries(densityMap)
    .filter(([, count]) => count > 0)
    .sort(([, a], [, b]) => b - a)
    .slice(0, topN)

  return sorted
    .map(([name, count]) =>
      buildNeighborhoodTooltipModel(name, count, densityMap, neighborhoodsGeoJSON, baltimore311Data, hideClosed)
    )
    .filter(Boolean)
}
